    ...
    <main className="min-h-screen overflow-hidden bg-(--color-bg) text-(--color-text)">
      ...
      <section className="landing-hero mx-auto grid max-w-7xl items-center gap-12 px-5 pb-16 pt-14 md:px-8 md:pb-24 md:pt-20 lg:grid-cols-[0.82fr_1.18fr] lg:gap-16 lg:px-10 lg:pt-24">
        <div className="relative z-10 max-w-xl">
          <motion.p 
            className="reveal-up flex items-center gap-2 text-sm font-medium"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6, delay: 0.1 }}
          >
            <AudioLines className="size-4 text-(--color-saffron)" /> 
            <span className="tricolor-text font-medium">Seamless Indian-language video dubbing, simplified</span>
          </motion.p>
          <motion.h1 
            className="landing-title reveal-up mt-6 font-serif text-[clamp(3.5rem,7vw,5.8rem)] leading-[0.92] tracking-[-0.04em]"
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.7, delay: 0.2 }}
          >
            Single video.<br /><span className="tricolor-text">Countless Indian voices.</span>
          </motion.h1>
          <motion.p 
            className="reveal-up mt-7 max-w-lg text-lg leading-8 text-(--color-text-dim)"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            <Link className="ui-button ui-button-tricolor min-h-11 px-5" to="/auth">
              Dub your first video <ArrowRight className="size-4" />
            </Link>
            <a className="inline-flex min-h-11 items-center gap-2 px-1 text-sm font-semibold transition hover:text-(--color-saffron)" href="#studio-preview">
              <Play className="size-4 fill-current" /> See the workflow
            </a>
          </motion.div>
          <motion.p 
            className="reveal-up mt-6 flex items-center gap-2 text-sm text-(--color-text-dim)"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6, delay: 0.5 }}
          >
            <Check className="size-4 text-(--color-green)" /> 
            Ten Indian languages covered. No middlemen or complicated editing workflows.
          </motion.p>
        </div>
      ...
      <section className="border-y border-(--color-border) bg-(--color-surface)" id="how-it-works">
        <div className="mx-auto grid max-w-7xl md:grid-cols-[0.9fr_1.1fr]">
          <div className="flex flex-col justify-between border-b border-(--color-border) px-5 py-12 md:border-b-0 md:border-r md:px-8 md:py-16 lg:px-10">
            <div>
              <p className="text-sm font-medium text-(--color-saffron)">From upload to Indian viewers</p>
              <h2 className="mt-4 max-w-md font-serif text-4xl leading-[1.02] tracking-[-0.03em] sm:text-5xl">
                Preserve your edit. <span className="tricolor-text">Connect in their language.</span>
              </h2>
            </div>
      ...
      <section className="mx-auto max-w-7xl px-5 py-20 md:px-8 md:py-28 lg:px-10">
        <div className="grid gap-12 lg:grid-cols-[0.78fr_1.22fr] lg:gap-20">
          <div>
            <h2 className="font-serif text-4xl leading-[1.03] tracking-[-0.03em] sm:text-5xl">
              Built for content <span className="tricolor-text">India will connect with.</span>
            </h2>
      ...
      <section className="bg-(--color-text) text-(--color-surface)">
        <div className="mx-auto flex max-w-7xl flex-col items-start justify-between gap-10 px-5 py-16 md:flex-row md:items-end md:px-8 md:py-20 lg:px-10">
          <div>
            ...
            <h2 className="mt-4 max-w-2xl font-serif text-4xl leading-[1.02] tracking-[-0.03em] sm:text-6xl">
              Let them experience your content in their native language.
            </h2>
          </div>
          <Link className="inline-flex min-h-12 shrink-0 items-center gap-3 bg-white px-5 text-sm font-semibold text-(--color-text) transition hover:bg-(--color-panel) ui-button-tricolor" to="/auth">
...
function StudioPreview() {
  return (
    ...
    >
      ...
      <div className="border border-(--color-text) bg-(--color-surface) glass-strong overflow-hidden">
        <div className="flex items-center justify-between border-b border-(--color-border) px-4 py-3">
          <div className="flex items-center gap-2">
            <span className="size-2 rounded-full bg-(--color-saffron) animate-pulse-saffron" />
            <span className="text-xs font-semibold">New dubbing job</span>
          </div>
        ...
        <div className="grid md:grid-cols-[1.2fr_0.8fr]">
          <div className="relative flex min-h-64 flex-col justify-between overflow-hidden bg-(--color-video) p-5 text-white md:min-h-[22rem]">
            <div className="flex items-center justify-between text-xs text-white/60">
              <span>lesson-final.mp4</span>
              <span>04:18</span>
            </div>
            <div className="relative z-10 mx-auto flex size-14 items-center justify-center rounded-full border border-white/30 bg-white/5">
              <Play className="ml-0.5 size-5 fill-current" />
            </div>
            <div>
              <div className="mb-3 flex items-end gap-1" aria-hidden="true">
                {[12, 28, 18, 36, 24, 42, 16, 34, 22, 38, 14, 30, 20, 40, 18, 28, 12, 32, 16, 24].map((height, index) => (
                  <motion.span 
              ...
              <div className="h-0.5 bg-white/20">
                <div className="h-full w-[62%] bg-(--color-saffron)" />
              </div>
            </div>
          </div>
          <div className="flex flex-col border-t border-(--color-border) md:border-l md:border-t-0">
            {stages.map((stage, index) => {
              ...
              return (
                <motion.div 
                  className="flex items-center gap-3 border-b border-(--color-border) px-4 py-4 last:border-0" 
                  key={stage.label}
                  whileHover={{ backgroundColor: "rgba(255, 153, 51, 0.05)" }}
                >
                  <span className="flex size-8 items-center justify-center bg-(--color-panel)">
                    <Icon className="size-3.5 text-(--color-text-dim)" />
                  </span>
                  <div>
                    <p className="text-xs text-(--color-muted)">{stage.label}</p>
                    <p className="mt-0.5 text-sm font-medium">{stage.value}</p>
                  </div>
                  {index < 3 ? (
                    <ChevronRight className="ml-auto size-3.5 text-(--color-muted)" />
                  ) : (
                    <Check className="ml-auto size-3.5 text-(--color-green)" />
                  )}
                </motion.div>
              );
            })}
            <div className="mt-auto p-4">
              <div className="flex gap-1" aria-hidden="true">
                {timeline.map((item, index) => (
                  <span className={`${item.opacity} h-1.5 bg-(--color-saffron)`} key={index} style={{ width: item.width }} />
                ))}
              </div>
              <Link className="mt-4 flex h-10 w-full items-center justify-center gap-2 bg-(--color-text) text-xs font-semibold text-white ui-button-tricolor" to="/auth">
                <Sparkles className="size-3.5" /> Start processing
              </Link>
            </div>
          </div>
        </div>
      </div>
      <div className="absolute -bottom-4 -left-3 flex items-center gap-2 border border-(--color-border) bg-(--color-bg) px-3 py-2 text-xs font-medium sm:-left-5">
...
function ProcessRow({ number, title, copy, icon: Icon }: { number: string; title: string; copy: string; icon: typeof Film }) {
  return (
    <motion.li 
      className="group grid gap-5 px-5 py-8 transition hover:bg-(--color-panel) sm:grid-cols-[3rem_1fr_auto] sm:items-start md:px-8 lg:px-10"
      whileHover={{ x: 5 }}
    >
      <span className="font-mono text-xs text-(--color-muted)">{number}</span>
      <div>
        <h3 className="text-lg font-semibold tracking-[-0.015em]">{title}</h3>
        <p className="mt-2 max-w-lg leading-7 text-(--color-text-dim)">{copy}</p>
      </div>
      <Icon className="hidden size-5 text-(--color-saffron) transition group-hover:translate-x-1 sm:block" />
    </motion.li>
  );
}
...
function AudienceRow({ title, copy, icon: Icon }: { title: string; copy: string; icon: typeof Users }) {
  return (
    <motion.div 
      className="grid gap-5 border-b border-(--color-border) py-8 sm:grid-cols-[3rem_0.7fr_1.3fr] sm:items-start"
      whileHover={{ x: 5 }}
    >
      <span className="flex size-9 items-center justify-center bg-(--color-panel)">
        <Icon className="size-4 text-(--color-saffron)" />
      </span>
      <h3 className="text-lg font-semibold">{title}</h3>
      <p className="max-w-xl leading-7 text-(--color-text-dim)">{copy}</p>
    </motion.div>
  );
}
...
export function Brand() {
  return (
    <Link className="flex items-center gap-2 text-(--color-text)" to="/">
      <span className="brand-mark">
        <img src="/src/assets/dubzy-logo.svg" alt="Dubzy" className="size-8" />
      </span>
      <span className="text-sm font-semibold tracking-[-0.02em] tricolor-text">{APP_NAME}</span>
    </Link>
  );
}